...
class Nixar:
  ...
  def __load_nixar_library(self):
    ...
    # init logging
    with Nixar.logging_init_lock:
      if not Nixar.is_logging_init:
  ...
  def __create_agent(self, alias: str, role: str, base64_seed: str, wallet_type: str = "sqlite",
    ...
    logging.info(f"Agent will be created with {json.dumps(self.mask_sensitive(init_params))}")
  ...
  def open_agent(self, alias: str, password_cb):
    self.agen_name = alias
    self.wallet_password_cb = self.__create_wallet_password_cb(password_cb)
  ...
  def get_attribute(self, attr_name: str, target_did: str) -> str:
    attr_name = self.__native_string(attr_name)
    target_did = self.__native_string(target_did)

    result = self.__externalize_sdk_result(
      self.nixar.get_attribute(self.agent_handle, attr_name, target_did))

    return result

  def get_public_did(self) -> dict:
    result = self.__externalize_sdk_result(self.nixar.get_public_did(self.agent_handle))
  ...
  def prover_store_credential(self, cred_id: str, credential: dict, client_nonce: str) -> str:
    cred_id = self.__native_string(cred_id)
    credential = self.__native_string(credential)
    client_nonce = self.__native_string(client_nonce)

    result = self.__externalize_sdk_result(
      self.nixar.prover_store_credential(self.agent_handle, cred_id, credential, client_nonce))

    return result

  def prover_delete_credential(self, cred_id: str) -> dict:
    cred_id = self.__native_string(cred_id)

    result = self.__externalize_sdk_result(
      self.nixar.prover_delete_credential(self.agent_handle, cred_id))
  ...
  def prover_get_credentials(self) -> list:
    result = self.__externalize_sdk_result(
      self.nixar.prover_get_credentials(self.agent_handle))

    return result

  def prover_get_credential_records(self) -> list:
    result = self.__externalize_sdk_result(
      self.nixar.prover_get_credential_records(self.agent_handle))
  ...
  def prover_create_presentation_by_ast(self, conditional_presentation_request: dict, self_attested_values: dict) -> dict:
    conditional_presentation_request = self.__native_string(conditional_presentation_request)
    self_attested_values = self.__native_string(self_attested_values)

    result = self.__externalize_sdk_result(
      self.nixar.prover_create_presentation_by_ast(self.agent_handle, conditional_presentation_request, self_attested_values))
  ...
  def verifier_verify_presentation_by_ast(self, conditional_presentation_request: dict, presentation: dict) -> bool:
    ...
    result = self.__externalize_sdk_result(
      self.nixar.verifier_verify_presentation_by_ast(self.agent_handle, conditional_presentation_request, presentation))
  ...
  def mask_sensitive(self, data, keys_to_mask=("password", "dbpassword", "seed", "base64seed",), mask="****"):
    if isinstance(data, dict):
      return {
        k: mask if k.lower() in keys_to_mask else self.mask_sensitive(v, keys_to_mask, mask)
        for k, v in data.items()
      }
    elif isinstance(data, list):
      return [self.mask_sensitive(item, keys_to_mask, mask) for item in data]
    else:
      return data
